...
The SVR model is a supervised learning model that is commonly used to predict discrete values. We felt that this would alogn with our project's goal since we intended to predict discrete unemployment rate values for each month in 2022. The initial dataset we fed into the model seemed to work and produce an accuracy score. But attempts thereafter to adjust the dataset ended up breaking the code for us. Due to the errors encountered in the preliminary accuracy prediction phase of the model, we were unable to proceed with the actual rate prediction. Additionally, we also could not find solutions to plotting the results as needed in the short time frame available. Therefore, this model did not end up being a useful model for our project. However, we believe that this model can potentially be useful and reliable down the road if we had more time to troubleshoot the code errors.  
